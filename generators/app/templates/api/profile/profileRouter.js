 ...
 *        id:
 *          type: string
 *          description: <% if (program === 'labs') { %>This is a foreign key (the okta user ID)<% } else {%>UUID or unique key of the user<% }%>
 *        email:
 *          type: string
 *        name:
 *          type: string
 ...
 *    summary: Get a list of all profiles<% if (program === 'labs') { %>
 *    security:
 *      - okta: []<% } %>
 *    tags:
 *      - profile
 ...
 *        $ref: '#/components/responses/UnauthorizedError'
 */
router.get('/', <% if (program === 'labs') { %>authRequired, <% } %>function (req, res) {
  Profiles.findAll()
    .then((profiles) => {
 ...
 *    summary: Returns a single profile<% if (program === 'labs') { %>
 *    security:
 *      - okta: []<% } %>
 *    tags:
 *      - profile
 ...
 */
router.get('/:id', <% if (program === 'labs') { %>authRequired, <% } %>function (req, res) {
 ...
 * /profile:
 *  post:
 *    summary: Add a profile<% if (program === 'labs') { %>
 *    security:
 *      - okta: []<% } %>
 *    tags:
 *      - profile
 ...
 *                  $ref: '#/components/schemas/Profile'
 */
router.post('/', <% if (program === 'labs') { %>authRequired, <% } %>async (req, res) => {
 ...
 * /profile:
 *  put:
 *    summary: Update a profile<% if (program === 'labs') { %>
 *    security:
 *      - okta: []<% } %>
 *    tags:
 *      - profile
 ...
 *                  $ref: '#/components/schemas/Profile'
 */
router.put('/', <% if (program === 'labs') { %>authRequired, <% } %>function(req, res) {
  const profile = req.body;
  if (profile) {
    const id = profile.id || 0;
    Profiles.findById(id)
 ...
 * /profile/{id}:
 *  delete:
 *    summary: Remove a profile<% if (program === 'labs') { %>
 *    security:
 *      - okta: []<% } %>
 *    tags:
 *      - profile
 ...
 *                  $ref: '#/components/schemas/Profile'
 */
router.delete('/:id', <% if (program === 'labs') { %>authRequired, <% } %>function(req, res) {
