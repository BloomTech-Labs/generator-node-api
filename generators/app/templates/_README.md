...
- `PORT` - API port (optional, but helpful with FE running as well)<% if (program === 'labs') { %>
...
- `DATABASE_URL` - connection string for postgres database<% if (program === 'labs') { %>
...
- run: `cp .en.sample .env` and update the environment variables to match your local project setup.
